...
export const argProcessorFactory = (short, long, processor) => args => (
  (args.includes(short)) ? processor(args, short)
    : (args.includes(long)) ? processor(args, long)
      : null
)

export const genericSingleArgProcessor = (args, flag) => {
...
export const multiArgNoFlag = args => {
...
export const genericMultiArgProcessor = (args, flag) => {
  const index = args.indexOf(flag);
